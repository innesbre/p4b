...
# Assignment 4 notes
* Do yourself and I a favour by submitted part 1 as an R script.  I know it says you don't need to, but if you accidentally submit a rich-text file, I'll be very sad, and you're writing R commands to text all your RegEx anyway, so just submit those so I can quickly run it and give you full marks.  
* For part 2, remember that a FASTA file is a text file where each sequence is preceded by a header line that starts with `>`.  You can use `readLines` to read in the file as a character vector, then loop over it to find each sequence by looking for each string that starts with `>`.  
